...
class Frog extends GameObject {
    constructor() {
        ...
        // Movement
        this._currentFactoryIndex = 0;
        this._factoryIndexDirection = 1;
    ...
    awake() {
        this.transform.position.x = window.factoryList[this._currentFactoryIndex].transform.position.x;
        this._moveSpeed = window.factoryList[1].transform.position.x - window.factoryList[0].transform.position.x;
    ...
    updateMovement() {
        ...
        // Turn move direction
        if (this._factoryIndexDirection == 1) {
            this._moveDirection = Vector2.right;
    ...
    jump() {
        ...
        this._isGround = false;
        this._jumping = true;
        if (this._currentFactoryIndex == window.factoryList.length - 1 && this._factoryIndexDirection == 1)
            this._factoryIndexDirection = -1;
        else if (this._currentFactoryIndex == 0 && this._factoryIndexDirection == -1)
            this._factoryIndexDirection = 1;
        this._currentFactoryIndex += this._factoryIndexDirection;
